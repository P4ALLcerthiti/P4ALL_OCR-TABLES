	...
	// Such areas include consequential type-2 and type-3 lines.
{
	...
	for (int i=1;i<Lines.size();i++)
	{
		...
		{
			if ((Lines_type[i-1]!=1)&&(Lines_type[i-1]!=4))
			{
				tmp.push_back(i);
			}
			else
...
void ocr_tabs::TableRows()
	...
	// without a type-2 line over them, they are not assigned to the table
{
	...
	for (int i=table_Columns.size()-1;i>=0;i--)
	{
		...
		for (int j=0;j<table_Columns[i].size();j++)
		{
			...
			for (int k=0;k<table_Columns[i][j].size()-1;k++)
			{
				...
				if ((abs(left1-left2)<=15)||(abs(right1-right2)<=15))
				{
					scrambled_col=true;
				}
	...
	// create table rows that include more than one lines
{
	...
	{
		if ((multi_Rows[i].size()<2)&&(multi_Rows[i][0].size()<2))
		{
			Lines_type[multi_Rows[i][0][0]]=1;
...
Mat ocr_tabs::ImagePreproccesing(Mat img)
{
	cout<< "Process Image...";
	start = clock();
	//A4 paper on 300dpi = 3500x2500px
	...
	{
		resize(img,img,cv::Size(img.size().width/((float)img.size().height/3500),3500));
	}
	Mat tmp;
	cv::GaussianBlur(img, tmp, cv::Size(0, 0), 3);
	cv::addWeighted(img, 1.5, tmp, -0.5, 0, test);
	duration = ( std::clock() - start ) / CLOCKS_PER_SEC;
	cout << " Done in "<<duration<< "s \n";
	return (img);
}
...
bool ocr_tabs::parsePDF(const std::string& filename, std::vector<cv::Mat>& imageList)
{
	...
	for (unsigned i=0;i<pagecount;i++)
	{
		...
		fz_write_png(ctx, pix, "tmp.png", 0);
		imageList.push_back(cv::imread("tmp.png",0));
